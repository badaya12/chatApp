...
export const AuthContextProvider = ({children}) =>{ 
    ...
    const changeStatus = useCallback(async(status)=>{
        try{
        const response = await postRequest(`${baseUrl}/users/changeStatus`, {status,userId:user._id});
        setUser({ ...user,status:status});
        }catch(error){
            alert("Can't be changed");
        }
    })
    return (<AuthContext.Provider value={{ user , changeStatus,registerInfo , updateRegisterInfo , registerUser , registerError , isRegisterLoading, logoutUser, loginUser, loginError, isLoginLoading, updateLoginInfo, loginInfo }}> 
        {children}
